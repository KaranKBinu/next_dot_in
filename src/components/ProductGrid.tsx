  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
      {products.map((product) => {
        ...
        return (
          <div key={product.id} className="group bg-white border border-[#E7E5E4] rounded-md p-4 flex flex-col justify-between hover:border-[#111827] transition-all relative">
            <button
              onClick={() => toggleFavorite(product.id)}
              className="absolute top-6 right-6 z-10 p-2 rounded-full bg-white/90 border border-[#E7E5E4] shadow-sm hover:scale-110 active:scale-95 transition-all"
              title={favorited ? "Saved to favorites" : "Add to favorites"}
            >
              <Heart className={`w-4 h-4 transition-colors ${favorited ? "fill-rose-600 text-rose-600" : "text-[#6B7280]"}`} />
            </button>
            ...
            <div className="mt-6 flex items-center justify-between pt-4 border-t border-[#E7E5E4]">
              ...
              <div className="flex gap-2">
                ...
                <Link
                  ...
                  className="px-3 py-2 bg-[#FAFAF8] text-[#111827] border border-[#E7E5E4] text-xs font-semibold uppercase tracking-wider rounded-md hover:border-[#111827] transition-colors"
                >
                  View
                </Link>
              </div>
