...
import { ShoppingBag, User as UserIcon, Heart, ShieldCheck, Search, Menu } from "lucide-react";
...
export default React.memo(function Header({
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center justify-between">
        <div className="flex items-center space-x-4 md:space-x-10">
          ...
          <button
            ...
            className="md:hidden p-2 min-h-[44px] min-w-[44px] flex items-center justify-center text-[#111827] hover:bg-[#F4F4F0] rounded-lg transition-colors active:scale-95 cursor-pointer"
        ...
        <div className="flex items-center space-x-2 sm:space-x-5">
          {/* Cmd+K Search Trigger Input (Desktop) */}
          <button
          ...
          <button
            onClick={() => setSearchModalOpen(true)}
            className="sm:hidden p-2 min-h-[44px] min-w-[44px] flex items-center justify-center text-[#6B7280] hover:text-[#111827] active:scale-95 transition-transform"
            title="Search"
          >
          ...
          {settings.wishlist_enabled && (
            <Link
              ...
              className="p-2 min-h-[44px] min-w-[44px] text-[#6B7280] hover:text-[#111827] transition-all hover:scale-105 active:scale-95 relative flex items-center justify-center group"
            >
              <Heart className="w-5 h-5 text-[#111827] fill-[#111827]/10 group-hover:scale-110 transition-transform" />
              {favoriteCount > 0 && (
                <span className="absolute top-1 right-1 w-4 h-4 bg-[#111827] text-white rounded-full text-[9px] font-extrabold flex items-center justify-center border border-white animate-heart-pop">
                  {favoriteCount}
                </span>
              )}
            </Link>
          ...
          >
            ...
            {itemCount > 0 && (
              <span className="absolute top-1 right-1 w-4 h-4 bg-[#111827] text-white rounded-full text-[9px] font-extrabold flex items-center justify-center border border-white animate-fade-in">
                {itemCount}
              </span>
            )}
          </Link>
          ...
          {user ? (
            ...
            <Link
              href="/login"
              className="hidden sm:inline-flex text-xs font-semibold uppercase tracking-wider px-4 py-2 bg-[#111827] text-white rounded hover:bg-[#27272A] active:scale-95 transition-all shadow-sm"
            >
              Sign In
            </Link>
          )}
      ...
      <MobileDrawer isOpen={mobileMenuOpen} onClose={closeMobileMenu} user={user} />
